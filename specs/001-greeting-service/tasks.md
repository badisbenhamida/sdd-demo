...
- [X] T1.1 Write tests/test_locales.py asserting exactly en, fr, de, es and ja load from config/locales.yml (GRT-007) [P1]
- [X] T1.2 Add config/locales.yml containing the five approved languages and their greeting text (GRT-007) [P1]
- [X] T1.3 Implement src/greeting_service/locales.py to load and validate the catalogue once at startup (GRT-007) [P1]
- [X] T1.4 Abort startup when config/locales.yml is missing, unparseable or has an empty greeting (GRT-007) [P1]
...
- [X] T2.1 Write tests/test_greeting.py covering every supported language returning its own greeting (GRT-001) [P1]
- [X] T2.2 Extend tests/test_greeting.py to assert two independent callers receive byte-identical text (GRT-002) [P1]
- [X] T2.3 Extend tests/test_greeting.py to assert a caller-supplied user identifier does not affect the response (GRT-006) [P1]
- [X] T2.4 Write tests/test_contract.py asserting one documented endpoint serves all callers with no per-caller variation (GRT-003) [P1]
- [X] T2.5 Implement the greeting endpoint in src/greeting_service/app.py returning catalogue text verbatim (GRT-001, GRT-002, GRT-003) [P1]
- [X] T2.6 Read the language from the request only, with no user lookup and no stored state (GRT-006) [P1]
...
- [X] T3.1 Write tests/test_errors.py asserting an unsupported language returns UNSUPPORTED_LANGUAGE with no greeting field (GRT-004) [P2]
- [X] T3.2 Extend tests/test_errors.py to assert a request with no language returns MISSING_LANGUAGE (GRT-008) [P2]
- [X] T3.3 Extend tests/test_errors.py to assert the two error codes are distinct and neither response carries fallback text (GRT-004, GRT-008) [P2]
- [X] T3.4 Declare the language parameter optional at framework level so FastAPI cannot pre-empt the error with its own 422 (GRT-008) [P2]
- [X] T3.5 Implement both error shapes in src/greeting_service/errors.py and wire them into the endpoint (GRT-004, GRT-008) [P2]
...
- [X] T4.1 Write tests/test_health.py asserting healthy is reported while a non-empty catalogue is loaded (GRT-005) [P3]
- [X] T4.2 Extend tests/test_health.py to assert an empty catalogue is not reported as healthy (GRT-005) [P3]
- [X] T4.3 Implement the health endpoint in src/greeting_service/app.py reporting on catalogue state (GRT-005) [P3]
